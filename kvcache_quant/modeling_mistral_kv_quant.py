...
class MistralAttention_KVQuant(nn.Module):
...
class MistralDecoderLayer_KVQuant(nn.Module):
    def __init__(self, config: MistralConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = MistralAttention_KVQuant(config=config, layer_idx=layer_idx)
...
class MistralModel_KVQuant(MistralPreTrainedModel):
    def __init__(self, config: MistralConfig):
        ...
        self.layers = nn.ModuleList(
            [MistralDecoderLayer_KVQuant(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
...
class MistralForCausalLM_KVQuant(MistralPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    _tp_plan = {"lm_head": "colwise_rep"}

    def __init__(self, config):
        super().__init__(config)
        self.model = MistralModel_KVQuant(config)
        self.vocab_size = config.vocab_size
